...
@Component({
  selector: 'app-new-students-principal',
  templateUrl: './new-students-principal.component.html',
  styleUrls: ['./new-students-principal.component.css'],
})
export class NewStudentsPrincipalComponent implements OnInit {
  constructor(
  ...
  submit() {
    ...
    const { doRequest: newStudent } = this.useRequestService.useRequest({
      url: '/api/students',
      method: HttpVerbs.POST,
    });
    newStudent(formData).subscribe(({ data, errors }) => {
      this.errors = errors || [];
